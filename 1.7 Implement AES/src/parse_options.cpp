...
byte* (*withInputData) (const byte* input, size_t in, const byte* key, size_t kn, bool log, const char* mode, size_t* outn) = nullptr;

void optionParsing(int argc, char** argv, byte* (*f) (const byte* input, size_t in, const byte* key, size_t kn, bool log, const char* mode, size_t* outn)) {
  ...
  if (mode == nullptr) {
    printf("ERROR - must specify mode with -m [encrypt|decrypt]\n");
    exit(0);
  }
  if ((strcmp(mode, "encrypt") != 0) && (strcmp(mode, "decrypt") != 0)) {
    printf("ERROR - must specify mode as \"encrypt\" or \"decrypt\", not \"%s\"\n", mode);
    exit(0);
  }
  ...
  if (inputF != HEXFILE_TOKENS) {
    size_t len;
    byte* output = (*withInputData)(_input, inputn, _input2, input2n, f_log, mode, &len);
...
void tokensFromHexFile (const char* hexData) {
  ...
  size_t len;
  byte* output = (*withInputData)(input, inputn, _input2, input2n, f_log, mode, &len);
  convertOutputFree(output, len, outputF);
  free(input);
}
