...
void reverseRows (byte* state) {
  for (short row = 1; row < 4; row++) {
    byte t0 = getByteFromState(state, row, 0),
         t1 = getByteFromState(state, row, 1),
         t2 = getByteFromState(state, row, 2),
         t3 = getByteFromState(state, row, 3);
    setByteInState(state, row, 0, t3);
    setByteInState(state, row, 1, t2);
    setByteInState(state, row, 2, t1);
    setByteInState(state, row, 3, t0);
  }
}
...
byte inline mul2 (byte b) {
  return (b << 1) ^ (0x1b & ((b >> 7)?0xff:0x00));
}
// Multiply by 3 -> multiply by 2 and Xor with original value.
byte inline mul3 (byte b) {
  return mul2(b) ^ b;
}
...
void invMixColumn (byte* state, short col) {
  //Implemented from the following reference: https://en.wikipedia.org/wiki/Rijndael_mix_columns
  //Addition is Xor in Rijndael's Galois field.
  /*
  ...
  */
  ...
  byte r0 = _14a_0 ^ _11a_1 ^ _13a_2 ^ _9a_3,
       r1 = _9a_0  ^ _14a_1 ^ _11a_2 ^ _13a_3,
       r2 = _13a_0 ^ _9a_1  ^ _14a_2 ^ _11a_3,
       r3 = _11a_0 ^ _13a_1 ^ _9a_2  ^ _14a_3;
  
  setByteInState(state, 0, col, r0);
  setByteInState(state, 1, col, r1);
  setByteInState(state, 2, col, r2);
  setByteInState(state, 3, col, r3);
}
...
byte* aes_encrypt_block (const byte* block_in, const byte* key, size_t kn, bool log) {
  ...
  for (unsigned short round = 0; round < rounds; round++) {
    ...
    if (log) {PAD printf("After addRoundKey:\n");showStateWithMoreIndent(state);}
    indent--;
  }
...
byte* aes_decrypt_block (const byte* block_in, const byte* key, size_t kn, bool log) {
  ...
  //Malloc the state, and initalize it with the ciphertext block
  byte* state = (byte*)malloc(sizeof(byte)*16);
  memcpy(state, block_in, sizeof(byte)*16);
  if (log) {PAD printf("Added ciphertext block:\n");showStateWithMoreIndent(state);}
  
  //Derive round keys and add the first round key in the initial round
  byte* keySchedule = expandKey(key, kn, log);
  addRoundKey(state, keySchedule+(rounds*kn), kn, log);
  if (log) {PAD printf("Added round key:\n");showStateWithMoreIndent(state);}
  
  //Perform all rounds, including final round
  for (unsigned short round = rounds-1; round > 0; round--) {
    if (log) {PAD printf("Round %d/%d\n", round+1, rounds);}
    indent++;
    if (log) {PAD printf("Initial state:\n");showStateWithMoreIndent(state);}
    ...
    if (round!=(rounds-1)) {
      invMixColumns(state);
      if (log) {PAD printf("After invMixColumns:\n");showStateWithMoreIndent(state);}
    }
    
    invShiftRows(state);
    if (log) {PAD printf("After invShiftRows:\n");showStateWithMoreIndent(state);}
    invSubBytes(state);
    if (log) {PAD printf("After invSubBytes:\n");showStateWithMoreIndent(state);}
    addRoundKey(state, keySchedule+(round*kn), kn, log);
    if (log) {PAD printf("After addRoundKey:\n");showStateWithMoreIndent(state);}
    indent--;
  }
  if (log) {PAD printf("Round 1/%d:\n", rounds);}
  indent++;
  invMixColumns(state);
  if (log) {PAD printf("After invMixColumns:\n");showStateWithMoreIndent(state);}
  invShiftRows(state);
  ...
  invSubBytes(state);
  if (log) {PAD printf("After invSubBytes:\n");showStateWithMoreIndent(state);}
  addRoundKey(state, keySchedule, kn, log);
  if (log) {PAD printf("After addRoundKey:\n");showStateWithMoreIndent(state);}
  indent--;
